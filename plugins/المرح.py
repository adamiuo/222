...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 5091307330:
        return await edit_or_reply(mention, f"**- لا يمكن استخدام الامر على المطور **")
    if user.id == 1614314857:
        return await edit_or_reply(mention, f"**- لا يمكن استخدام الامر على المطور**")
    if user.id == 5269601471:
        return await edit_or_reply(mention, f"**- لا يمكن استخدام الامر على المطور**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
        mention, f"- المستخدم [{tag}](tg://user?id={user.id}) \n تـم رفـعه شاذ هـنا "
...
async def permalink(mention):
    ...
    await edit_or_reply(
        mention,
        f"- المستخدم [{tag}](tg://user?id={user.id}) \n تم رفعها مراتك بنجاح الف مبروك 😹🤤",
    )
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 5091307330:
        return await edit_or_reply(mention, f"**- لا يمكن استخدام الامر على المطور زلمة وعلى راسك**")
    if user.id == 1614314857:
        return await edit_or_reply(mention, f"**- لا يمكن استخدام الامر على المطور زلمة وعلى راسك**")
    if user.id == 5269601471:
        return await edit_or_reply(mention, f"**- لا يمكن استخدام الامر على المطور زلمة وعلى راسك**")
