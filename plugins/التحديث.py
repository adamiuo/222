...
async def upstream(event):
    ...
    if ENV and (HEROKU_API_KEY is None or HEROKU_APP_NAME is None):
        return await edit_or_reply(
            event, "⌔∮ يجب عليك وضع الفارات المطلوبة لتحديث جولد"
        )
    try:
        txt = (
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n⌔∮ عزيز المستخدم انت تستخدم اخر اصدار من جولد 🫂♥"
        )
        return repo.__del__()
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"ارسل `{cmdhd}تحديث الان` لتحديث سورس جولد"
        )
    ...
    if conf == "الان":
        await event.edit("⌔∮ جارِ تحديث جولد يرجى الأنتظار قليلا")
