...
async def Logo(event):
    ...
    if not text:
        ...
        return
    fnt = await get_font_file(event.client, "@usersource")
    if event.reply_to_msg_id:
        rply = await event.get_reply_message()
        logo_ = await rply.download_media()
    else:
        async for i in bot.iter_messages(f"@usersource", filter=InputMessagesFilterPhotos):
            PICS_STR.append(i)
    ...
    await bot.send_file(
        event.chat_id, file_name, caption=f"⪼ تم صنعه بواسطه : {mention} [@AKJA0]"
    )
    await evxnt.delete()
    try:
        os.remove(file_name)
